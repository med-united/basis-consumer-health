...
- [X] T016 [US1] Implement card-attribute reading in `CardAttributeReader` (EF.GDO→ICCSN; EF.Version2→CardVersionInfo tolerant TLV; AUT cert→cardHolderName/certExpirationDate/KVNR; unreadable→null per FR-005/FR-007) in `apdu-lib/src/main/java/de/servicehealtherx/apdu/card/CardAttributeReader.java`; wired into `CardObjectFactory.readAndCreate`. **eGK AUT cert (TUC_KON_001 §2c)**: `read(EGK)` selects DF.ESIGN by AID and reads `EF.C.CH.AUT.E256` (ECC, preferred) with fallback to `EF.C.CH.AUT.R2048` (RSA) via multi-block READ BINARY (`readEgkAutCertificate`/`readBinaryFull`, sized from the leading ASN.1 SEQUENCE); cardHolderName=CN, certExpirationDate=notAfter, KVNR=`[A-Z]\d{9}` OU from the subject DN (`extractKvnr`). **Note**: HBAx (`C.HP.AUT`) and SMC-B (`C.HCI.AUT`) AUT-cert paths still deferred — for those types cert fields are null unless the provider supplies the DER explicitly.
