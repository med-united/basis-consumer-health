...
**Branch**: `002-card-handle` | **Date**: 2026-06-07 (updated 2026-06-14) | **Spec**: [spec.md](spec.md)
...
| Dependency | Purpose |
|---|---|
| `apdu-lib` (this repo) | Transport-neutral home of CM_CARD_LIST type, domain objects, and the `CardReaderPort` abstraction; no `sicct-lib` dependency |
| `javax.smartcardio` (JDK) | **PC/SC transport**: directly connected reader access; `CardTerminals.waitForChange()`/`list()` polling for insert/remove detection (drives `PcscCardReaderPort`) |
| Quarkus 3.x + quarkus-cxf | CDI lifecycle, SOAP endpoint hosting |
| Netty 4.x | **SICCT transport**: SICCT TCP channel; card insert/remove events arrive via `SicctChannelHandler` (drives `SicctCardReaderPort`) |
| JPA + H2 | G2CardLog admin log (persisted); CM_CARD_LIST instances are in-memory only |
...
| JUnit 5 + Quarkus Test | Unit and integration tests; Netty EmbeddedChannel for SICCT simulation; a fake `CardReaderPort` for PC/SC simulation |
| api-telematik (submodule) | WSDL/XSD-generated types for EventService v7.2, CardTerminalService v1.1 |

**Storage**:
- **CardObject / CardSession / AuthState** — runtime in-memory only; held in each provider's `CmCardList` (`ConcurrentHashMap<String, CardObject>`); never persisted to DB. Rebuilt from reader/terminal query on startup and reconnect.
- **G2CardLog** — JPA entity persisted to H2; tracks G2.0 SMC-B/HBA pseudonym + last insertion timestamp per ICCSN (A_25801).
- **cardHandle 48-hour blacklist** — in-memory `ConcurrentLinkedDeque<BlacklistedHandle>` inside each `CmCardList`; bounded to 10,000 entries; cleared on restart (acceptable — restart itself issues new handles).

**Testing**:
- Unit: JUnit 5; direct construction of `CmCardList`, `CardObjectFactory` with a fake `CardReaderPort` (no transport, no hardware) — exercises the transport-agnostic core for both transports through one test double
- Integration: `@QuarkusTest`; Netty `EmbeddedChannel` for the SICCT path; a `CardReaderPort` test double / PC/SC reader simulator for the PC/SC path; embedded H2 for G2CardLog; CXF test client for GetCards / RequestCard / EjectCard SOAP operations
- Each card-handle scenario is run over BOTH transports (parameterised) to assert parity (SC-018)
- All SICCT and PC/SC hardware interactions replaced by `CardReaderPort`-based test doubles
...
**Performance Goals**:
- Card Handle creation: ≤ 2 s from insertion detection on BOTH transports (FR-001 / SC-001); PC/SC reader polling interval chosen to fit inside the 2 s budget
- GetCards response: ≤ 100 ms (in-memory aggregation across providers' lists, FR-041/FR-064)
- Startup reconstruction (all readers + terminals): ≤ 10 s after last reader/terminal connects (SC-013)
- Disconnect/unplug invalidation: ≤ 1 s on both transports (SC-014, FR-069)
...
| Principle I — Code Quality: no `Impl` suffix | ✅ PASS | All classes named by role: `CmCardList`, `CardObjectFactory`, `PcscCardReaderPort`, `SicctCardReaderPort`, `SicctEventPublisher` |
| Principle II — Testing discipline | ✅ PASS | Unit tests for `CmCardList` ops via fake `CardReaderPort`; IT tests for SOAP + both transport flows; parity tests across PC/SC + SICCT (SC-018) |
| Principle III — Security: no handle bytes in logs | ✅ PASS | cardHandle logged only at DEBUG, never in ERROR/WARN |
| Principle IV — Resource constraints (150 MB ceiling) | ✅ PASS | ConcurrentHashMap per-entry ≈ 2 KB; 200 slots × 2 KB = 400 KB — negligible (per-provider lists, summed) |
| Principle V — CDI standard DI | ✅ PASS | Providers are `@ApplicationScoped`; each owns its own (non-singleton) `CmCardList`; events via CDI `Event<T>` |
