...
**Module placement**: the card-management domain objects (CM_CARD_LIST, CardObject/CardHandle, CardVersion, CardType, CardSession and its subtypes, AuthState) and the transport-neutral lifecycle logic live in the **`apdu-lib`** module so they carry no transport dependency (`apdu-lib` MUST NOT depend on `sicct-lib`). A single shared **CM_CARD_LIST** instance is owned jointly by — and is part of — both the **PCSC Crypto Provider** (`PcscCryptoProvider`, backed by a directly connected PC/SC reader) and the **Sicct Crypto Provider** (`SicctCryptoProvider`, backed by a SICCT terminal). Both providers register CardObjects into the same list, giving one unified card view across both transports.
...
When a smart card is inserted into a directly connected PC/SC reader, the system applies the same Card Handle creation logic as for SICCT and registers the CardObject into the same shared CM_CARD_LIST. The handle is indistinguishable from a SICCT-originated handle to client systems — the terminal ID (`ctid`) identifies the source reader, but the handle format and fields are identical.
...
**Independent Test**: Insert a card into a directly PC/SC-connected reader. Confirm a Card Handle is created with the same field structure as a SICCT-originated handle, that the CardObject is added to the shared CM_CARD_LIST, and that GetCards returns it under the same response schema. Exercise RequestCard and EjectCard against the PC/SC reader and confirm equivalent behaviour to the SICCT path.
...
### User Story 3 — Unified CM_CARD_LIST Across Both Transports (Priority: P1)

The PCSC Crypto Provider and the Sicct Crypto Provider share a single CM_CARD_LIST instance. Cards inserted via PC/SC readers and cards inserted via SICCT terminals appear together in one list, so GetCards returns a unified, transport-spanning view and any `cardHandle` resolves regardless of which provider created it.

**Why this priority**: a single source of truth for known cards prevents divergent or duplicated card state between the two providers and lets higher layers address any card by handle without knowing its transport.

**Independent Test**: With one card in a SICCT terminal and one card in a directly PC/SC-connected reader simultaneously, call GetCards once and confirm both CardObjects are returned from the same CM_CARD_LIST. Resolve each `cardHandle` and confirm the correct originating provider/transport handles subsequent APDU operations.

**Acceptance Scenarios**:

1. **Given** one card present in a SICCT terminal and one card present in a directly PC/SC-connected reader, **When** GetCards is queried, **Then** both CardObjects are returned from a single CM_CARD_LIST with identical field structure and distinct `ctid` values.
2. **Given** a `cardHandle` resolved from CM_CARD_LIST, **When** an APDU-level operation is issued, **Then** it is routed to the provider (PCSC or SICCT) that owns the originating reader, transparently to the caller.
3. **Given** a card is removed from either transport, **When** the removal is detected, **Then** the corresponding CardObject is removed from the shared CM_CARD_LIST and no longer appears in GetCards.
...
- **FR-001**: When a card is inserted in a SICCT terminal slot, the system MUST create a Card Handle within 2 seconds of the insertion event being received and add the CardObject to CM_CARD_LIST.
...
- **FR-062**: Exactly one CM_CARD_LIST instance MUST be shared by, and be part of, both the PCSC Crypto Provider (`PcscCryptoProvider`) and the Sicct Crypto Provider (`SicctCryptoProvider`). Both providers MUST register their CardObjects into, and remove them from, this same instance.
...
- **FR-064**: GetCards MUST return a unified view spanning both transports from the single CM_CARD_LIST; a card reached via PC/SC and a card reached via SICCT MUST be indistinguishable in structure, differing only in `ctid` (and capability-dependent fields).
- **FR-065**: A `cardHandle` resolved from CM_CARD_LIST MUST route subsequent APDU execution to the provider that owns the originating reader (PCSC or SICCT), transparently to the caller. The provider boundary is the only place transport-specific APDU transmission occurs; `apdu-lib` constructs APDUs and owns CM_CARD_LIST but MUST NOT transmit APDUs itself.
...
- **FR-067**: `cardHandle` uniqueness, the 48-hour no-reuse rule, and per-tenant scoping (FR-003, FR-019) MUST hold across the unified CM_CARD_LIST regardless of which transport created each entry.
...
- **CM_CARD_LIST**: The card-management list of the Kartendienst (gemSpec_Kon §4.1.5) — the single, transport-agnostic registry of every CardObject currently known to the system. Indexed by `cardHandle` and by (`ctid`, `slotNo`). Maintained by TUC_KON_001 on insertion (add CardObject) and by the card-removal reaction (remove CardObject). Lives in `apdu-lib`; one shared instance is part of both `PcscCryptoProvider` and `SicctCryptoProvider`. Enforces `cardHandle` uniqueness, the 48-hour no-reuse blacklist, and per-tenant scoping. Independent of the underlying transport (PC/SC reader or SICCT terminal).
...
- **SC-017**: With one card in a SICCT terminal and one in a directly PC/SC-connected reader simultaneously, a single GetCards call returns both CardObjects from one CM_CARD_LIST; zero duplicate or divergent entries across the two providers in a test of 20 concurrent mixed-transport insertions.
...
- A single CM_CARD_LIST instance is shared by both providers (e.g. via dependency injection of the same application-scoped bean), giving one unified card view across transports.
...
- Q: Is CM_CARD_LIST per-provider or shared? → A: A single shared instance is part of both `PcscCryptoProvider` and `SicctCryptoProvider`, so GetCards returns one unified card view across both transports and any `cardHandle` resolves regardless of originating provider.
