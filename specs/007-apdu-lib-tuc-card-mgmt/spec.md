...
`apdu-lib` is an APDU construction library only: it generates command APDUs and expected response constraints. It MUST NOT transmit APDUs to cards. APDU execution is owned by transport modules such as `sicct-lib` and `crypto-pcsc-lib`.
...
- **FR-002**: Each TUC implementation MUST enforce all preconditions defined in its specification (card type restrictions, lock state checks, PIN status checks, authentication state checks) before generating the APDU flow.
- **FR-003**: Each TUC implementation MUST return all output parameters needed by callers to execute the flow (generated APDUs, expected status words, semantic metadata such as result mappings), as specified in the corresponding TAB_KON table.
...
- **FR-009**: TUC_KON_208 (Send Secured APDU) MUST validate the sequence number in the signed scenario and reject replayed or out-of-order scenarios before generating the secured APDU request sequence.
...
- **FR-015**: Each TUC MUST be independently testable without a card terminal by validating generated APDU bytes and expected status metadata.
- **FR-016**: The `apdu-lib` module MUST NOT depend on `sicct-lib` and MUST NOT include transport abstractions that send APDUs.
- **FR-017**: APDU objects generated by `apdu-lib` MUST use JDK `javax.smartcardio.CommandAPDU` as the canonical command representation.
...
- **CommandAPDU**: `javax.smartcardio.CommandAPDU` objects generated by `apdu-lib` for later execution by the transport layer.
- **ExpectedStatusWord / ExpectedStatusSet**: Status-word expectations attached to generated APDUs for deterministic response evaluation by callers.
...
- **SC-004**: PIN entry operations (TUC_KON_012, TUC_KON_019, TUC_KON_021) generate complete APDU command sequences (including PIN-pad display related commands) within 50ms under normal conditions.
- **SC-005**: APDU generation operations (TUC_KON_200, TUC_KON_208) produce a complete command payload in under 20ms for a single request.
...
- Card terminal communication is provided by existing infrastructure layers (`sicct-lib`, `crypto-pcsc-lib`); `apdu-lib` only constructs APDUs and expected response metadata.
- `apdu-lib` has no compile-time dependency on `sicct-lib`.
