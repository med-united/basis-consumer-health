...
- **FR-096**: All SICCT card terminal configuration — name, host, port, and per-terminal timeouts (`connectTimeoutMs`, `apduTimeoutMs`, `maxRetries`, `initialBackoffMs`, `maxBackoffMs`) — MUST be stored exclusively as `CardTerminal` JPA entity fields in the JavaDB; no `quarkus.sicct.terminals[*]` config keys exist. Global SICCT extension settings (worker thread pool size, reconnect watchdog interval) MUST be stored in the `AppConfigProperty` table and read via the `configsource-db` ConfigSource per FR-028.
...
- **FR-222**: The `quarkus-sicct-extension` runtime (`de.servicehealtherx.quarkus.sicct.runtime.jmx`) MUST expose `SicctTerminalDiscoveryManagementMBean` / `SicctTerminalDiscoveryManagement` (`de.servicehealtherx:module=quarkus-sicct-extension,name=SicctTerminalDiscoveryManagement`) with:
  ...
  - `getLastDiscoveryResult(): String` — returns a JSON snapshot of the most recently discovered terminals: `[{ "terminalId": "...", "host": "...", "port": 4876, "discoveredAt": "..." }]`
...
- **CardTerminal**: A persisted SICCT terminal configuration. Fields: unique name (per tenant), host, port, optional per-terminal timeouts, tenant association, creation and update timestamps. Stored in the database; managed exclusively via the management console.
...
- All production application configuration is stored in the JavaDB `AppConfigProperty` table and read via `configsource-db` (FR-028); no production-relevant configuration resides in filesystem config files. SICCT card terminal configurations (host, port, timeouts, name) are stored exclusively as `CardTerminal` JPA entities; `quarkus.sicct.terminals[*]` config keys do not exist in this system.
