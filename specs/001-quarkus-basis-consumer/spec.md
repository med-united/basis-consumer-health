...
- Each component MUST be an independent Maven module with its own folder. The mandatory module structure is:
  ...
  - **`crypto-lib`** — depends on `de.gematik.pki/gemLibPki@4.0.2` (includes Bouncy Castle); downloads and manages the TI Trust Service List (TSL); parses certificates and extracts attributes (registration number, SMC-B/eHBA type); exposes the `CryptoProvider` CDI interface and shared `KeyStoreAdapter` base classes; exposes CDI injectable beans
  - **`crypto-p12-lib`** — depends on `crypto-lib`; implements `CryptoProvider` as `P12CryptoProvider` (`@ApplicationScoped`) backed by PKCS#12 keystores; registered as a CDI bean so `Instance<CryptoProvider>` in `crypto-services-lib` discovers it automatically
  - **`crypto-pcsc-lib`** — depends on `crypto-lib`; implements `CryptoProvider` as `PcscCryptoProvider` (`@ApplicationScoped`) backed by local PC/SC card readers via `javax.smartcardio`; registered as a CDI bean
  - **`crypto-sicct-lib`** — depends on `crypto-lib` and `sicct-lib`; implements `CryptoProvider` as `SicctCryptoProvider` (`@ApplicationScoped`) backed by networked SICCT card terminals; registered as a CDI bean
  - **`sicct-lib`** — depends on `crypto-lib` and `javax.smartcardio`; contains `SICCT.asn1` for jASN1 code generation (classes generated during Maven `generate-sources`); implements all EHEALTH AUTHENTICATE action variants (CREATE, VALIDATE, ADD Phase 1/2); implements JCE `KeyStoreSPI` for SMC-B and eHBA including required APDU generation; exposes CDI injectable beans. Contains JPA beans for lifecycle management of card terminals.
  ...
  - **`crypto-services-lib`** — depends on `crypto-lib` only (MUST NOT have a dependency on `sicct-lib`); implements `SignatureService`, `CertificateService`, and `EncryptionService` independent of any middleware; injects `Instance<CryptoProvider>` to discover all registered providers at runtime; uses CDI
