...
**Persistence layer**: JPA + JavaDB (Apache Derby). JPA entities live in `sicct-lib`. Runtime-only state is held in `@ApplicationScoped` CDI beans. Configuration entities are read from MicroProfile Config at startup.
